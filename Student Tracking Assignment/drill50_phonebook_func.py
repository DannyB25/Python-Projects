...
def first_run(self): #creating first_run function and feeding in self to give function access to all class objects 
    data = ('John', 'Doe', 'John Doe', '111-111-1111', 'jdoe@email.com', 'JavaScript') #providing values 
    conn = sqlite3.connect('phonebook.db')
    with conn:
        cur = conn.cursor() #connection cursor as the variable cur 
        cur,count = count_records(cur) 
        if count < 1:
            cur.execute("""INSERT INTO tbl_phonebook (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoe@email.com','JavaScript'))
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and(len(var_email) > 0) and(len(var_course) > 0): # enforce the user to provide both names
        ...
        with conn:
            ...
            if chkName == 0: # if this is 0 then there is no existance of the fullname and we can add new data
                ...
                cursor.execute("""INSERT INTO tbl_phonebook (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""",(var_fname,var_lname,var_fullname,var_phone,var_email,var_course))
...
def onUpdate(self):
    ...
    if (len(var_phone) > 0) and (len(var_email) > 0) and (len(var_course) > 0): # ensure that there is data present
        ...
        with conn:
            ...
            print(count2)
            cur.execute("""SELECT COUNT(col_course) FROM tbl_phonebook WHERE col_course = '{}'""".format(var_course))
            ...
            if count == 0 or count2 == 0 or count3 == 0: # if proposed changes are not already in the database, then proceed
                ...
                if response:
                    ...
                    with conn:
                        cursor = conn.cursor()
                        cursor.execute("""UPDATE tbl_phonebook SET col_phone = '{0}',col_email = '{1}' WHERE col_fullname = '{2}'""".format(var_phone,var_email,var_course,var_value))
                        onClear(self)
                        conn.commit()
                else:
